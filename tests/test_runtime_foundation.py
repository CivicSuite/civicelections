...
def test_root_reports_honest_current_state():
    ...
    assert payload["status"] == "election administration foundation plus workpaper persistence"
    assert "database-backed candidate/canvass workpapers" in payload["message"]
...
def test_api_endpoints_return_deterministic_payloads():
    ...
    assert client.post("/api/v1/civicelections/candidate-filing", json={"office":"Mayor","requirements":["Petition"]}).json()["filing_id"] is None
    ...
    assert client.post("/api/v1/civicelections/canvass-checklist", json={"election_name":"2027","items":["Reconcile"]}).json()["canvass_id"] is None
