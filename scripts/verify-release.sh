#!/usr/bin/env bash
set -euo pipefail
echo "VERIFY-RELEASE: CivicElections v0.1.1"
PYTHON_CANDIDATES=(); [[ -n "${CIVICELECTIONS_RELEASE_PYTHON:-}" ]] && PYTHON_CANDIDATES+=("$CIVICELECTIONS_RELEASE_PYTHON")
...
import hashlib
expected={"civicelections-0.1.1-py3-none-any.whl","civicelections-0.1.1.tar.gz"}
...
PY
"$PYTHON_BIN" - <<'PY'
import civicelections
assert civicelections.__version__ == "0.1.1"
