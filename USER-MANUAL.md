...
For IT: install with `python -m pip install -e ".[dev]"` and run with `python -m uvicorn civicelections.main:app --host 127.0.0.1 --port 8141`. Depends on `civiccore==0.3.0`.
